...
pub fn sanitize_historical_model_suffixes(home: &Path) -> anyhow::Result<SanitizeModelSuffixResult> {
...
fn sanitize_logs_model_suffixes(home: &Path) -> anyhow::Result<()> {
    ...
    let mut stmt = conn.prepare("SELECT rowid, feedback_log_body FROM logs WHERE feedback_log_body LIKE '%[%'")?;
...
mod tests {
    ...
    fn strips_trailing_suffix_from_model_names() {
        ...
        assert_eq!(
            sanitize_model_suffixes_in_text("glm-5.2[1M]"),
            "glm-5.2"
        );
    }
