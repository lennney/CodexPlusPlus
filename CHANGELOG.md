...
- 模型列表支持在模型名后加 `[1M]`、`[200K]` 或 `[1000000]` 等后缀指定上下文窗口；不写后缀的模型仍使用 Codex 默认长度。
- 同一模型同时出现带后缀和不带后缀版本时，统一按带后缀版本的窗口长度处理。
- 启动时自动清理历史 session 数据库与 Local Storage 中残留的带后缀模型名，避免模型选择器显示废弃的历史项。
